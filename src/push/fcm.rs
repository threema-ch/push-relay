...
fn get_fcm_uri(project_id: String, fcm_authority: impl AsRef<str>) -> String {
...
impl FcmEndpointConfig {
    fn new(config: &config::FcmConfig, fcm_authority: impl AsRef<str>) -> Self {
        let endpoint = get_fcm_uri(config.project_id.clone(), fcm_authority);
        Self {
...
pub mod test {
    ...
    pub fn get_fcm_test_path(config: &config::FcmConfig) -> String {
        get_fcm_uri(config.project_id.clone(), "")
    }
